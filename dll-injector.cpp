...
int arg_check(int argc, char** argv)
{
    if (argc != 4)
    {
        std::cout << "\nUsage (first option) : " << argv[0] << " example.exe mydll.dll\n";
        std::cout << "Second option: " << argv[0] << " -i 12345 mydll.dll\n";
        std::cout << "specify process to inject in\n";
        std::cout << "specify assembly to inject\n";
        std::cout << "\n--------------------\nWARNING: high recommendation to use -i option as first parameter and the process ID after that\nIt is a lot more secure than specifying process name.";
        std::cout << "\nyou can get process id for example using process hacker\n";
        return 1;
    }
    if (strcmp(argv[1], "-i") == 0) {
        use_id = true;
        procID = std::stoi(argv[2]);
    }
    else {
        use_id = false;
        proc_name = argv[2];
